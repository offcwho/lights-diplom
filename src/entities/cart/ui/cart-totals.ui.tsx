...
export const CartTotalsUi = ({ className, style }: { className?: string; style?: React.CSSProperties }) => {
    ...
    return (
        <>
            {items.length > 0 && (
                <div
                    className={`lg:col-span-5 bg-white border border-black/5 rounded-3xl p-6 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.03)] space-y-6 sticky lg:top-16 ${className}`}
                    style={style}
                >
                    <div className="space-y-6 lg:block xs:hidden">
                        ...
                        <div className="flex justify-between items-baseline">
                            ...
                            <motion.span
                                key={total} // Заставляет цифру мягко подпрыгивать при изменении
