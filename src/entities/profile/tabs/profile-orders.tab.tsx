...
export const ProfileOrdersTab = () => {
    ...
    useEffect(() => {
        ordersApi.list().then((data) => setOrders(data))
    }, [user]);
        ...
        <>
            ...
            <div className="">
                {orders && orders.length > 0 ? (
                        ...
                        <div className="bg-[#F5F4F1] rounded-3xl p-4 sm:p-6 border border-black/5 flex flex-col sm:flex-row justify-between sm:items-center gap-3 sm:gap-4">
                            <div className="min-w-0">
                                <div className="flex items-center flex-wrap gap-2">
                                    ...
                                    <span className="text-[10px] bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-semibold">{item.status}</span>
                                </div>
                    ...
                    <div className="bg-[#F5F4F1] rounded-3xl p-4 sm:p-6 border border-black/5">
                        <p className="text-xs text-zinc-500">Вы еще не сделали ни одного заказа</p>
                    </div>
